...
# ---- Branding defaults (updated at runtime by refresh_branding()) -----------
# Before a server type is selected these provide safe generic values.
# After the first-run dialog or config DB load, refresh_branding() overwrites
# them with values from the server-type registry.
APP_NAME = "Drive"
COMPANY = "Hyland"
...
# SYNC_ROOT has moved to the server-type registry (nuxeo/registration.py).
# Nuxeo-specific code should use: st.get("NUXEO").sync_root
...
def refresh_branding(server_type_key: str = "") -> None:
    """Re-compute branding globals from the server-type registry.

    Called once the server type is known (after the first-run dialog or
    after reading it from the config DB).  All modules that already
    imported ``APP_NAME``, ``COMPANY``, etc. will see the old values
    in their local namespace, but any future ``from constants import …``
    and any code that accesses ``constants.APP_NAME`` will pick up the
    new values.
    """
    global APP_NAME, COMPANY, BUNDLE_IDENTIFIER, NXDRIVE_SCHEME, CONFIG_REGISTRY_KEY

    from nxdrive.drive import server_type as _st

    key = server_type_key or _st.get_default_key()
    config = _st.get(key)

    APP_NAME = config.app_name
    COMPANY = config.company
    BUNDLE_IDENTIFIER = config.bundle_identifier
    NXDRIVE_SCHEME = config.url_scheme
    CONFIG_REGISTRY_KEY = config.config_registry_key
