...
class LocalClient(LocalClientMixin):
    ...
    @staticmethod
    def _emblem_name() -> str:
        from nxdrive.drive import server_type as _st
        from nxdrive.drive.options import Options

        return _st.get(Options.server_type).emblem_name

    def has_folder_icon(self, ref: Path, /) -> bool:
        """Check if the folder icon is set."""
        name = self._emblem_name()
        emblem = self.shared_icons / f"{name}.svg"
        ...
        matcher = re.compile(rf"metadata::emblems: \[.*{re.escape(name)}.*\]")
