...
from nxdrive.drive.options import DEFAULT_LOG_LEVEL_FILE, Options
from nxdrive.drive.osi import AbstractOSIntegration
from nxdrive.drive.poll_workers import (
...
if TYPE_CHECKING:
    ...
    from nxdrive.drive.updater import Updater  # noqa
...
class Manager(QObject):
    ...
    def __init__(self, home: Path, /) -> None:
        ...
        self._engine_types: Dict[str, Type[Engine]] = self._build_engine_types()
    ...
    @staticmethod
    def _build_engine_types() -> Dict[str, Type["Engine"]]:
        """Build the engine type registry from server-type registrations."""
        import importlib

        types: Dict[str, Type["Engine"]] = {}
        for config in st.all_configs().values():
            module_path, class_name = config.engine_class_path.rsplit(".", 1)
            try:
    ...
    def _apply_server_type_config(self) -> None:
        ...
        that are not available for the selected server type."""
        server_type = self.get_config("server_type")
        if server_type:
    ...
    def _generate_csv_async(self, engine: Engine, session: Session) -> None:
        from nxdrive.drive.session_csv import (  # Circular import with Manager otherwise
            SessionCsv,
        )
    ...
    def _detect_server_type(url: str, /) -> str:
        """Detect the server type from the URL using registered URL patterns.

        Each server-type registration provides a list of URL path suffixes.
        The first config whose pattern matches the URL path wins.
        If nothing matches, the default server type is used.
        """
        config = st.detect_by_url(url)
