...
from nxdrive.drive.auth import Token, get_auth
from nxdrive.drive.client.proxy import get_proxy
...
from nxdrive.drive.objects import Binder, DocPair
from nxdrive.drive.options import Options
from nxdrive.drive.qt import constants as qt
from nxdrive.drive.qt.imports import QObject, QUrl, Signal, Slot
...
class QMLDriveApi(QObject):
    openAuthenticationDialog = Signal(str, object)
    setMessage = Signal(str, str)
    downloadLocationChanged = Signal()
    showReloginPopup = Signal(str, str)  # (engine_uid, username)
    ...
    def change_download_location(self) -> None:
        ...
        from nxdrive.drive.qt.imports import QFileDialog
    ...
    def _get_ssl_error_for_server(
        self, server_url: str, login_page: str = "", /
    ) -> str:
        """Validate a server URL by probing the given *login_page*.

        Uses the ``ssl_login_page`` from the server-type registry when
        *login_page* is not explicitly provided.  SSL certificate errors
        are handled identically for all server types.
        """
        kwargs = {"proxy": self._manager.proxy}
        if login_page:
            kwargs["login_page"] = login_page
        try:
            return test_url(server_url, **kwargs)
        except InvalidSSLCertificate as exc:
            log.warning(exc)
            parts = urlsplit(server_url)
            hostname = parts.netloc or parts.path
            if self.application.accept_unofficial_ssl_cert(hostname):
                ...
                if Options.ca_bundle:
                    saved_conf["ca_bundle"] = Options.ca_bundle
                save_config(saved_conf)
                return self._get_ssl_error_for_server(server_url, login_page)
        except MissingClientSSLCertificate as exc:
            log.warning(exc)
            return "MISSING_CLIENT_SSL"
        except EncryptedSSLCertificateKey as exc:
    ...
    def default_local_folder_for_server(self, server_type_key: str, /) -> str:
        """Return the default local folder name for a given server type."""
        config = st.get(server_type_key)
        return str(get_default_local_folder(config.local_folder_name))
    ...
    def relogin(self, engine_uid: str, password: str, /) -> None:
        ...
        engine_config = (
            st.get_by_engine_type(getattr(engine, "type", "")) if engine else None
        )
        if (
            not engine
            or not engine_config
            or engine_config.supports_browser_token_update
        ):
            self.setMessage.emit("CONNECTION_UNKNOWN", "error")
            return
    ...
    def web_authentication(
        ...
        # Only server types that support browser-based auth need the login page check
        if use_legacy_auth and detected_config.supports_browser_token_update:
            try:
        ...
        try:
            ...
            server_type_key = st.get_by_engine_type(engine_type).key
            auth = get_auth(
                server_url,
                crafted_token,
                dao=self._manager.dao,
