"""Generic tree-model items for folder/document selection dialogs.

Server-specific subclasses and document providers (e.g. Nuxeo's ``FoldersOnly``)
...
class FoldersOnlyBase:
    """Abstract base for server-specific folder tree providers.

    Used by the Direct Transfer feature to browse and select a remote
    destination folder.  Subclasses must implement ``get_top_documents``
    and ``get_children``.
    """
    ...
    def get_top_documents(self) -> Iterator["Documents"]:
        """Fetch all top-level documents / roots."""
...
class FilteredDocuments:
    ...
    def __init__(self, remote: Any, filters: Filters, /) -> None:
        self.remote = remote
