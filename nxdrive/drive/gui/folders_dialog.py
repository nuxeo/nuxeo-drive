...
class FoldersDialog(DialogMixin):
        ...
        for local_path in paths:
            ...
            if path.is_dir():
                ...
                current_total_size = self._process_file(
                    path, current_total_size, file_limit, folder_limit, skipped_items
                )
    ...
    def _process_directory(
        ...
        try:
            ...
            # Skip if adding this folder would exceed the total size limit
            if folder_limit and (current_total_size + total_dir_size) > folder_limit:
                skipped_items.append(path.name)
                return current_total_size
    ...
    def _process_file(
        ...
        try:
            ...
            # Skip if adding this file would exceed the total size limit
...
class ReviewSelectionDialog(QDialog):
    ...
    def __init__(self, folder_dialog: "FoldersDialog", /) -> None:
        ...
        # The selection list
        self.model = FileModel(folder_dialog.paths, self)
        ...
        self.tree_view.sortByColumn(FileModel.NAME, Qt.SortOrder.AscendingOrder)
        for column in (FileModel.NAME, FileModel.TYPE, FileModel.SIZE):
