...
from nxdrive.drive.constants import APP_NAME, COMPANY, MAC, WINDOWS
...
def check_executable_path_error_qt(path: Path, /) -> None:
    ...
    from nxdrive.drive.qt.imports import QApplication, QMessageBox, QPixmap
    from nxdrive.drive.translator import Translator
    from nxdrive.drive.utils import find_icon, find_resource
...
def fatal_error_qt(exc_formatted: str, /) -> None:
    ...
    def open_update_site() -> None:
        ...
        config = _st.get(Options.server_type)
        if WINDOWS:
            exe = config.download_exe
        elif MAC:
            exe = config.download_dmg
        else:
            exe = config.download_appimage
...
def show_critical_error() -> None:
    ...
    with suppress(Exception):
        ...
        from nxdrive.drive.options import _get_nxdrive_home

        crash_file = _get_nxdrive_home(Path.home()) / "crash.state"
