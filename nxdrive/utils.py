...
def is_generated_tmp_file(name: str) -> Tuple[bool, Optional[bool]]:
    ...
    Options is used and values may cahnged the time the app is running.
...
def normalized_path(path_mixed: Union[bytes, str, Path]) -> Path:
    """ Return absolute, normalized file path. """

    if isinstance(path_mixed, bytes):
        path = Path(force_decode(path_mixed))
    elif isinstance(path_mixed, str):
        path = Path(path_mixed)
    else:
        path = path_mixed
...
def normalize_event_filename(filename: Union[str, Path], action: bool = True) -> Path:
    ...
    Note: we cannot decorate the function with lru_cache() as is because it as
