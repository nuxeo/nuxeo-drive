...
class Processor(_ProcessorBase):
    ...
    def _handle_doc_pair_sync(self, doc_pair: DocPair, sync_handler: Callable) -> None:
        ...
        if status is not DigestStatus.OK:
            # Some server types (e.g. Alfresco) do not carry content
            # digests in their metadata, so a None/empty digest is
            # expected.  Only enforce the digest gate when the current
            # engine belongs to the default (Nuxeo) server type.
            from nxdrive.drive import server_type as _st

            engine_cfg = _st.get_by_engine_type(getattr(self.engine, "type", ""))
            if engine_cfg.key == _st.get_default_key():
