...
class EngineDAO(ConfigurationDAO):
    ...
    def get_session_uploads(self, session_id: int) -> List[Upload]:
        """
    ...
    def get_completed_sessions_raw(
        ...
        con = self._get_read_connection()
        c = con.cursor()
        sql_req = "SELECT * FROM Sessions WHERE status = ? OR status = ?"
        if formatted:
            sql_req += " ORDER BY created_at DESC LIMIT 20"
        return [
            {
                "uid": res.uid,
                "status": TransferStatus(res.status),
            ...
            for res in c.execute(
                sql_req, (TransferStatus.DONE.value, TransferStatus.CANCELLED.value)
            ).fetchall()
        ]
