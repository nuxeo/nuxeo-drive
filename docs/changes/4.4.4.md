...
- Added `QMLDriveApi.close_direct_transfer_window()`
...
- Added `Application.destroyed_server_folders()`
- Added `Application.get_direct_transfer_remote_path_url()`
- Added `is_direct_transfer` attribute to `objects.py::Upload`
- Added `view.py::DirectTransferModel`
- Changed `EngineDAO.save_upload()` to handle new column
