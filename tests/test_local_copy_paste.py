...
class TestLocalCopyPaste(UnitTestCase):
    ...
    def setUp(self):
        ...
        for file_num in range(
            ...
            filename = self.FILE_NAME_PATTERN % (
                file_num,
                os.path.splitext(self.TEST_DOC_RESOURCE)[1],
            )
    ...
    def _local_copy_paste_files(self, stopped=False):
        ...
        ) % (len(children_1), self.folder_path_1, num)
        ...
        if not cond2:
            ...
            error2 = (
                'file names in "%s" are different, e.g. duplicate files '
                "(renamed)\nunexpected files:\n%s\n\nmissing "
                "files\n%s"
            ) % (self.folder_path_1, extra, missing)
        ...
        error3 = (
            'number of local files (%d) in "%s" is different ' "from original (%d)"
        ) % (len(children_2), self.folder_path_2, num)
        ...
        if not cond4:
            ...
            error4 = (
                ...
                "(renamed)\nunexpected files:\n%s\n\nmissing "
                "files\n%s"
            ) % (self.folder_path_2, extra, missing)
        ...
        ) % (len(remote_children_1), remote_ref_1_name, num)
        ...
        if not cond6:
            ...
            error6 = (
                'remote file names in "%s" are different, e.g. '
                "duplicate files (renamed)\nunexpected files:"
                "\n%s\n\nmissing files\n%s"
            ) % (remote_ref_1_name, extra, missing)
        ...
        error7 = (
            'number of remote files (%d) in "%s" is different ' "from original (%d)"
        ) % (len(remote_children_2), remote_ref_2_name, num)
        remote_expected = set(self.local_files_list)
        ...
        if not cond8:
            ...
            error8 = (
                ...
                "duplicate files (renamed)\nunexpected files:"
                "\n%s\n\nmissing files\n%s"
            ) % (remote_ref_2_name, extra, missing)
