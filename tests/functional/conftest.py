...
from nuxeo.documents import Document as Doc
from nuxeo.exceptions import BadQuery
from nuxeo.models import Blob, Document, FileBlob, User
from nuxeo.users import User as Usr
...
def user_factory(request, server, faker):
    ...
    def _make_user(username: str = "", password: str = env.NXDRIVE_TEST_PASSWORD):
        ...
        user = server.users.create(Usr(properties=properties))
...
def obj_factory(request, server, tmp_path):
    ...
    def _make(
        ...
        new = Doc(name=title, type=nature, properties={"dc:title": title})
...
def attach_blob(nuxeo: Nuxeo, doc: Doc, file: Path) -> Blob:
    # Upload the file
