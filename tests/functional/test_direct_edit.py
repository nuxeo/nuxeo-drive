...
def test_cleanup_orphan_files(manager_factory, obj_factory):
    """
    Files marked as locked must be ignored.
    These files are removed after there are unlocked.
    """
    manager, engine = manager_factory()
    ...
    with manager:
        direct_edit = manager.direct_edit
        direct_edit._folder.mkdir()
        doc = obj_factory(title="this is a orphan file.txt", nature="File")

        folder = direct_edit._folder / f"{doc.uid}_file-content"
        folder.mkdir()

        file = folder / "this is a orphan file.txt"
        file.touch()
        file.write_text("bla" * 3, encoding="utf-8")
        assert file.is_file()
