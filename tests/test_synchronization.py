...
class TestSynchronization(UnitTestCase):
    ...
    def test_binding_initialization_and_first_sync(self):
        ...
        assert remote.get_content(self._duplicate_file_1), 'Some content.'
        assert remote.get_content(self._duplicate_file_2), 'Other content.'
    ...
    def test_binding_synchronization_empty_start(self):
        ...
        folder_count, file_count = self.get_local_child_count(
    ...
    def test_create_content_in_readonly_area(self):
        ...
        assert sorted_states[2].pair_state in ('locally_created',
                                               'unsynchronized')
        assert sorted_states[3].local_name == 'Sub Folder 1'
        assert sorted_states[3].pair_state in ('locally_created',
                                               'unsynchronized')
        assert sorted_states[4].local_name == 'File 2.txt'
        assert sorted_states[4].pair_state in ('locally_created',
                                               'unsynchronized')
        ...
        assert sorted_states[5].pair_state in ('locally_created',
                                               'unsynchronized')
    ...
    def test_synchronize_special_filenames(self):
        ...
        file_ = remote.make_file(folder, u'File with chars: / \\ * < > ? "',
                                 content='some content', doc_type='Note')
        ...
        assert local.get_content(
            u'/Folder with chars- - - - - - - -'
            u'/File with chars- - - - - - - -.txt') == 'new content'
    ...
    def test_synchronize_windows_foldername_endswith_space(self):
        ...
        self.remote_1.make_file(target.uid, 'aFile.txt', u'File A Content')
        self.remote_1.make_file(target.uid, 'bFile.txt', u'File B Content')
        self.engine_1.start()
        self.wait_sync(wait_for_async=True)
        ...
        if AbstractOSIntegration.is_windows():
            assert local.exists(
                ...
                "Folder 'trial ' should be created " \
                "without trailing space in the name"
            assert local.exists(
            ...
            assert local.exists(
                '/Nuxeo Drive Test Workspace/trial /'),\
                "Folder 'trial ' should be created " \
                "with trailing space in the name"
            assert local.exists(
                '/Nuxeo Drive Test Workspace/trial /aFile.txt'),\
                "trial/aFile.txt should sync"
            assert local.exists(
