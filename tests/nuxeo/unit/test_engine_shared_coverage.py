...
class TestResumeSession:
    ...
    def test_resume_session(self, mock_engine):
        ...
        # Avoid the scheduled-session code path which spawns a QDialog
        # (would abort with no QApplication in scope).
        mock_session = Mock()
        mock_session.scheduled_at = 0
        mock_engine.dao.get_session = Mock(return_value=mock_session)
    ...
    def test_unbind(self, mock_engine, tmp_path):
        ...
        with patch.object(mock_engine, "stop") as mock_stop:
            with patch.object(mock_engine, "dispose_db") as mock_dispose:
                with patch("nxdrive.drive.engine.engine.shutil"):
                    mock_engine.unbind()
...
class TestCheckLastSync:
    ...
    def test_check_last_sync_completed_no_errors(self, mock_engine):
        ...
        with patch("nxdrive.drive.engine.engine.datetime"):
            mock_engine._check_last_sync()
...
class TestStop:
    ...
    def test_stop(self, mock_engine):
        ...
        with patch("nxdrive.drive.engine.engine.Processor") as mock_processor:
            mock_engine.stop()
