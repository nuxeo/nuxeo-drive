...
from tests.common.functional.mocked_classes import Mock_DAO, Mock_Local_Client
...
def test_execute(manager_factory):
    ...
    with patch(
        ...
        assert (
            ex.exconly()
            == "nxdrive.drive.exceptions.ThreadInterrupt: dummy_interrupt_process"
        )
...
def test_scan_handle_deleted_files(manager_factory):
    ...
    with patch("nxdrive.drive.engine.engine.Engine.delete_doc") as mock_delete:
