...
def test_custom_memory_handler():
    ...
    test_logger = logging.getLogger("nxdrive.test.memory_handler")
    ...
    try:
        ...
        assert buffer[0].getMessage() == "Line n° 50,000"
        assert buffer[-1].getMessage() == "Line n° 99,999"
        ...
        assert buffer[0].getMessage() == "Line n° 99,958"
        assert buffer[-1].getMessage() == "Line n° 99,999"
    finally:
...
def test_sensitive_data_masking(tmp_path):
    """Test sensitive log masking for defined nxdrive.drive.logging_filter_patterns.patterns"""
