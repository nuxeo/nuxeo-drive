...
def _clear_module_cache():
    ...
    keys_to_remove = [
        ...
        if k.startswith("nxdrive.drive.auth") and k != "nxdrive.drive.auth.base"
    ]
...
def test_get_auth_uses_auth_factory_from_config():
    ...
    mock_factory.assert_called_once_with(
        "http://host", {"access_token": "abc"}, extra="val"
    )
...
def test_get_auth_fallback_to_token_authentication():
    ...
    with patch(
        "nxdrive.drive.server_type.get_default_key", return_value="NUXEO"
    ), patch("nxdrive.drive.server_type.get", return_value=mock_config), patch(
        "nxdrive.drive.auth.token.TokenAuthentication", mock_token_auth_cls
    ):
        from nxdrive.drive.auth import get_auth
...
def test_getattr_oauthentication_returns_loaded_class():
    ...
    with patch(
        ...
        "nxdrive.drive.server_type.load_class", return_value=mock_klass
    ):
        from nxdrive.drive import auth
...
def test_getattr_oauthentication_fallback_to_base():
    ...
    with patch(
        ...
        "nxdrive.drive.server_type.load_class", return_value=None
    ):
        from nxdrive.drive import auth
