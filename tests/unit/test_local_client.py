...
def test_get_children_info(local):
    ...
    assert workspace_children[0].path == Path(file_1.name)
...
def test_get_info_invalid_date(local):
    ...
    doc_1_info = local.get_info(doc_1, check=False)
    assert doc_1_info.name == "Document 1.txt"
    assert doc_1_info.path == Path(doc_1.name)
...
def test_is_equal_digests(local):
    ...
    # Equal digests
    assert local.is_equal_digests(local_digest, local_digest, local_path, check=False)
    ...
    assert not local.is_equal_digests(
        local_digest, remote_digest, local_path, check=False
    )

    # Different digests with different digest algorithms but same content
    remote_digest = hashlib.sha1(content).hexdigest()
    assert local_digest != remote_digest
    assert local.is_equal_digests(local_digest, remote_digest, local_path, check=False)
...
def test_make_documents(local):
    doc_1 = local.make_file(ROOT, "Document 1.txt")
    assert local.exists(local.get_path(doc_1))
    doc_1_info = local.get_info(doc_1, check=False)
    assert doc_1_info.name == "Document 1.txt"
    assert doc_1_info.path == Path(doc_1.name)
    assert doc_1_info.get_digest() == EMPTY_DIGEST
    ...
    assert local.exists(local.get_path(doc_2))
    doc_2_info = local.get_info(doc_2, check=False)
    assert doc_2_info.name == "Document 2.txt"
    assert doc_2_info.path == Path(doc_2.name)
    assert doc_2_info.get_digest() == SOME_TEXT_DIGEST
    assert not doc_2_info.folderish
    assert doc_2_info.size > 0

    local.delete(local.get_path(doc_2))
    assert local.exists(local.get_path(doc_1))
    assert not local.exists(local.get_path(doc_2))
    ...
    assert not local.exists(local.get_path(doc_3))
