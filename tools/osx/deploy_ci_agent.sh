...
create_package() {
    ...
    if [ "${SIGNING_ID:-unset}" != "unset" ]; then
        ...
        echo ">>> [sign] Verifying code signature1 for QtQuickControls2"
        codesign --verbose=4 --deep --strict "${pkg_path}/Contents/Frameworks/PyQt5/Qt/lib/QtQuickControls2.framework/Versions/5/QtQuickControls2"
        echo ">>> [sign] Verifying framework QtQml"
