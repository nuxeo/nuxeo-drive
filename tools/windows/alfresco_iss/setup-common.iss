...
[Run]
; Launch Alfresco Drive after installation
...
[Code]
function IsAlfrescoDriveRunning(): Boolean;
// Check if Alfresco Drive is currently running by querying the task list.
...
begin
    Result := False;
    TempFile := ExpandConstant('{tmp}\alfresco_drive_check.txt');
    Exec(
...
// Before setup begins, check whether Alfresco Drive is running.
// If it is, ask the user for confirmation:
//   - Yes  → kill Alfresco Drive and continue installation.
