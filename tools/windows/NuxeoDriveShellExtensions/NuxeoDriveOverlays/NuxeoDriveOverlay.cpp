...
NuxeoDriveOverlay::NuxeoDriveOverlay(): _communicationSocket(0), _referenceCount(1)
{
}

NuxeoDriveOverlay::~NuxeoDriveOverlay(void)
{
}

IFACEMETHODIMP_(ULONG) NuxeoDriveOverlay::AddRef()
{
	return InterlockedIncrement(&_referenceCount);
}

IFACEMETHODIMP NuxeoDriveOverlay::QueryInterface(REFIID riid, void** ppv)
{
...
IFACEMETHODIMP_(ULONG) NuxeoDriveOverlay::Release()
{
...
IFACEMETHODIMP NuxeoDriveOverlay::GetPriority(int* pPriority)
{
	pPriority = 0;
...
IFACEMETHODIMP NuxeoDriveOverlay::IsMemberOf(PCWSTR pwszPath, DWORD dwAttrib)
{
...
IFACEMETHODIMP NuxeoDriveOverlay::GetOverlayInfo(PWSTR pwszIconFile, int cchMax, int* pIndex, DWORD* pdwFlags)
{
	*pIndex = 0;
...
bool NuxeoDriveOverlay::_IsOverlaysEnabled()
{
	int* enable = new int();
	bool success = false;
...
bool NuxeoDriveOverlay::_IsMonitoredFileState(const wchar_t* filePath)
{
	bool needed = false;
