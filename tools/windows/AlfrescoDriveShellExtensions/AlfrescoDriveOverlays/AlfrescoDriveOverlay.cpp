 ...
 * Copyright (c) 2025 Hyland Software, Inc. and its affiliates. All rights reserved. All Hyland product names are registered or unregistered trademarks of Hyland Software, Inc. or its affiliates.
 *
...
#include "AlfrescoDriveOverlay.h"
...
AlfrescoDriveOverlay::AlfrescoDriveOverlay(): _communicationSocket(0), _referenceCount(1)
{
}

AlfrescoDriveOverlay::~AlfrescoDriveOverlay(void)
{
}

IFACEMETHODIMP_(ULONG) AlfrescoDriveOverlay::AddRef()
{
	return InterlockedIncrement(&_referenceCount);
}

IFACEMETHODIMP AlfrescoDriveOverlay::QueryInterface(REFIID riid, void** ppv)
{
...
IFACEMETHODIMP_(ULONG) AlfrescoDriveOverlay::Release()
{
...
IFACEMETHODIMP AlfrescoDriveOverlay::GetPriority(int* pPriority)
{
	pPriority = 0;
...
IFACEMETHODIMP AlfrescoDriveOverlay::IsMemberOf(PCWSTR pwszPath, DWORD dwAttrib)
{
...
IFACEMETHODIMP AlfrescoDriveOverlay::GetOverlayInfo(PWSTR pwszIconFile, int cchMax, int* pIndex, DWORD* pdwFlags)
{
	*pIndex = 0;
...
bool AlfrescoDriveOverlay::_IsOverlaysEnabled()
{
	int* enable = new int();
	bool success = false;
...
bool AlfrescoDriveOverlay::_IsMonitoredFileState(const wchar_t* filePath)
{
	bool needed = false;
