...
class LocalClient(BaseClient):
    ...
    @register()
    def ignore_non_existent(self, parent, name, **kwargs):
    ...
    def ignore_prefix_and_suffix(self, parent, name, **kwargs):
        """
        Ignore files with the matching prefix and suffix (with same position inthe tuple). Examples:
        Office temp file: see http://support.microsoft.com/kb/211632
        Emacs auto save file: see http://www.emacswiki.org/emacs/AutoSave
        :param parent: parent relative path
        :param name: file name
        :param kwargs: normally the registration parameters (prefixes, suffixes,...)
        :return: True is the file should be ignored, False otherwise
        """
        prefixes = kwargs['prefixes']
        suffixes = kwargs['suffixes']
    ...
    def is_ignored(self, parent_ref, file_name):
        ...
        if dir_name:
            ignore = any(f(self, grandparent_ref, dir_name) for f in registry if f.apply_to_parent)
        else:
            ignore = False
