...
if __name__ == "__main__":
    ...
    benchmark_empty = u'/Users/looping/nuxeo/sources/nuxeo/addons/nuxeo-drive/tools/benchmark/empty'
    folder = benchmark_empty
    options = namedtuple('options',['nxdrive_home'])
    options.nxdrive_home = "/Users/looping/.nuxeo-drive/"
    configure(log_filename="/tmp/trace.log", use_file_handler=True, file_level='TRACE', console_level='DEBUG')
    manager = Manager(options)
    binder = namedtuple('binder',['username','password','url'])
    binder.username = 'Administrator'
    binder.password = 'Administrator'
    binder.url = 'http://localhost:8080/nuxeo/'
    engine = manager.get_engine(folder)
    if engine is None:
        engine = manager.bind_engine('NXDRIVE', folder, binder)
    manager.start()
