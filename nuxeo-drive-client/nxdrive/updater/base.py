...
class BaseUpdater(PollWorker):
    ...
    def __init__(self, manager):
        ...
        self.enable = getattr(self, '_can_update', getattr(sys, 'frozen', False))
    ...
    def _download(self, version):
        ...
        try:
            with requests.get(url, stream=True, verify=False) as req, open(path, 'wb') as tmp:
    ...
    def _fetch_versions(self):
        ...
        try:
            ...
            log.exception('Impossible to poll update site URL')
            raise UpdateError('Impossible to get %r: %s' % (url, exc))
        else:
            content = req.text
