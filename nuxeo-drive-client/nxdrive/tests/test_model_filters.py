...
class TestModelFilter(IntegrationTestCase):
    ...
    def testSimpleFilter(self):
        ...
        server_binding=ServerBinding(self.local_test_folder_1,self.nuxeo_url, self.admin_user)
        Filter.add(session, server_binding, "/Test/Plop")
        self.assertEqual(len(Filter.getAll(session, server_binding)), 1, "Save of one filter has failed")
        # On purpose to verify that filter not messup if starts with the same value
        Filter.add(session, server_binding, "/Test/Plop2")
        self.assertEqual(len(Filter.getAll(session, server_binding)), 2, "Save of second filter has failed")
        Filter.add(session, server_binding, "/Test/Plop2/SubFolder")
        self.assertEqual(len(Filter.getAll(session, server_binding)), 2, "Save of a filter already filtered has failed")
        Filter.remove(session, server_binding, "/Test/Plop2/SubFor")
        self.assertEqual(len(Filter.getAll(session, server_binding)), 2, "Remove non existing filter has failed")
