...
class ConfigurationDAO(QObject):
    ...
    def __init__(self, db):
        ...
        if migrate:
            res = c.execute(
                "SELECT value FROM Configuration WHERE name='{}'".format(
                    SCHEMA_VERSION)).fetchone()
            schema = int(res[0]) if res else 0
            if schema != self.schema_version:
                self._migrate_db(c, schema)
        else:
            c.execute('INSERT INTO Configuration(name,value) VALUES(?,?)',
    ...
    def _create_configuration_table(self, cursor):
        cursor.execute('CREATE TABLE if not exists Configuration(name '
                       'VARCHAR NOT NULL, value VARCHAR, PRIMARY KEY (name))')
    ...
    def update_config(self, name, value):
        with self._lock:
            ...
            c = con.cursor()
            if value:
                c.execute('UPDATE OR IGNORE Configuration SET value=? '
    ...
    def get_config(self, name, default=None):
        ...
        obj = c.execute('SELECT value FROM Configuration WHERE name=?',
                        (name,)).fetchone()
...
class ManagerDAO(ConfigurationDAO):
    ...
    def _init_db(self, cursor):
        ...
        cursor.execute(
            'CREATE TABLE if not exists Engines(uid VARCHAR, '
            'engine VARCHAR NOT NULL, name VARCHAR, '
            'local_folder VARCHAR NOT NULL UNIQUE, PRIMARY KEY(uid))')
        cursor.execute(
            'CREATE TABLE if not exists Notifications(uid VARCHAR UNIQUE, '
            'engine VARCHAR, level VARCHAR, title VARCHAR, '
            'description VARCHAR, action VARCHAR, flags INT, '
            'PRIMARY KEY(uid))')
        cursor.execute(
            'CREATE TABLE if not exists AutoLock(path VARCHAR, '
            'remote_id VARCHAR, process INT, PRIMARY KEY(path))')
    ...
    def insert_notification(self, notification):
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                'INSERT INTO Notifications(uid, engine, level, title, '
                'description, action, flags) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ...
    def lock_path(self, path, process, doc_id):
        with self._lock:
            try:
                ...
                c.execute('UPDATE AutoLock SET process=?, remote_id=? '
    ...
    def update_notification(self, notification):
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                'UPDATE Notifications SET level = ?, title = ?, '
    ...
    def get_notifications(self, discarded=True):
        ...
        if discarded:
            ...
            return c.execute(
                'SELECT * FROM Notifications WHERE (flags & {}) = 0'.format(
    ...
    def discard_notification(self, uid):
        ...
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            c.execute(('UPDATE Notifications SET flags = (flags | {0}) '
                       'WHERE uid=? AND (flags & {1}) = {1}').format(
    ...
    def _migrate_db(self, cursor, version):
        if version < 2:
            cursor.execute(
                'CREATE TABLE if not exists Notifications(uid VARCHAR, '
                'engine VARCHAR, level VARCHAR, title VARCHAR, '
                'description VARCHAR, action VARCHAR, flags INT, '
                'PRIMARY KEY(uid))')
    ...
    def update_engine_path(self, engine, path):
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            c.execute('UPDATE Engines SET local_folder=? WHERE uid=?',
                      (path, engine))
            if self.auto_commit:
                con.commit()
    ...
    def add_engine(self, engine, path, key, name):
        with self._lock:
            ...
            if self.auto_commit:
                con.commit()
            result = c.execute(
                'SELECT * FROM Engines WHERE uid=?', (key,)).fetchone()
        return result
...
class EngineDAO(ConfigurationDAO):
    ...
    def _migrate_db(self, cursor, version):
        if version < 1:
            ...
            cursor.execute(
                "UPDATE States SET last_transfer = 'upload' "
                'WHERE last_local_updated < last_remote_updated '
                'AND folderish=0;')
            cursor.execute(
                "UPDATE States SET last_transfer = 'download' "
                'WHERE last_local_updated > last_remote_updated '
                'AND folderish=0;')
            self.update_config(SCHEMA_VERSION, 1)
        if version < 2:
            cursor.execute('CREATE TABLE if not exists ToRemoteScan'
                           '(path STRING NOT NULL, PRIMARY KEY(path))')
            self.update_config(SCHEMA_VERSION, 2)
        if version < 3:
            self._migrate_state(cursor)
            self.update_config(SCHEMA_VERSION, 3)
    ...
    def _create_state_table(cursor, force=False):
        ...
        # Should improve that
        cursor.execute(
            'CREATE TABLE {} States(id INTEGER NOT NULL, last_local_updated '
            'TIMESTAMP, last_remote_updated TIMESTAMP, local_digest VARCHAR, '
            'remote_digest VARCHAR, local_path VARCHAR, remote_ref VARCHAR, '
            'local_parent_path VARCHAR, remote_parent_ref VARCHAR, '
            'remote_parent_path VARCHAR, local_name VARCHAR, '
            'remote_name VARCHAR, size INTEGER DEFAULT (0), '
            "folderish INTEGER, local_state VARCHAR DEFAULT('unknown'), "
            "remote_state VARCHAR DEFAULT('unknown'), pair_state VARCHAR "
            "DEFAULT('unknown'), remote_can_rename INTEGER, "
            'remote_can_delete INTEGER, remote_can_update INTEGER, '
            'remote_can_create_child INTEGER, last_remote_modifier VARCHAR, '
            'last_sync_date TIMESTAMP, error_count INTEGER DEFAULT (0), '
            'last_sync_error_date TIMESTAMP, last_error VARCHAR, '
            'last_error_details TEXT, version INTEGER DEFAULT (0),'
            'processor INTEGER DEFAULT (0), last_transfer VARCHAR,'
            'PRIMARY KEY (id), UNIQUE(remote_ref, remote_parent_ref), '
    ...
    def _init_db(self, cursor):
        ...
        for table in ('Filters', 'RemoteScan', 'ToRemoteScan'):
            cursor.execute(
                ...
                '(path STRING NOT NULL, PRIMARY KEY(path))'.format(table))
    ...
    def release_processor(self, processor_id):
        with self._lock:
            ...
            c.execute('UPDATE States SET processor=0 WHERE processor=?',
                      (processor_id,))
            if self.auto_commit:
                con.commit()
        return c.rowcount > 0
    ...
    def acquire_processor(self, thread_id, row_id):
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            c.execute('UPDATE States SET processor=? WHERE id=? '
                      'AND (processor=0 OR processor=?)',
    ...
    def reinit_processors(self):
        with self._lock:
            ...
            c.execute(
                'UPDATE States SET error_count=0, last_sync_error_date=NULL, '
                "last_error = NULL WHERE pair_state='synchronized'")
            if self.auto_commit:
                con.commit()
            con.execute('VACUUM')
    ...
    def delete_remote_state(self, doc_pair):
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            update = ("UPDATE States SET remote_state='deleted', "
                      'pair_state=?')
    ...
    def delete_local_state(self, doc_pair):
        ...
        try:
            with self._lock:
                ...
                parent = c.execute('SELECT * FROM States WHERE local_path=?',
                ...
                update = ("UPDATE States SET local_state='deleted', "
                          'pair_state=?')
                c.execute('{} WHERE id=?'.format(update),
    ...
    def insert_local_state(self, info, parent_path):
        ...
        with self._lock:
            ...
            c.execute(
                'INSERT INTO States(last_local_updated, local_digest, '
                'local_path, local_parent_path, local_name, folderish, '
                'size, local_state, remote_state, pair_state) '
    ...
    def get_last_files(self, number, direction=''):
        ...
        return c.execute(
            "SELECT * FROM States WHERE pair_state = 'synchronized' "
            'AND folderish = 0 {} ORDER BY last_sync_date'
            'DESC LIMIT {}'.format(condition, number)).fetchall()
    ...
    def register_queue_manager(self, manager):
        ...
        with self._lock:
            ...
            pairs = c.execute(
                'SELECT * FROM States WHERE {} ORDER BY local_path ASC'.format(
    ...
    def update_last_transfer(self, row_id, transfer):
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            c.execute('UPDATE States SET last_transfer=? WHERE id=?',
                      (transfer, row_id))
            if self.auto_commit:
                con.commit()
    ...
    def get_dedupe_pair(self, name, parent, row_id):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE id != ? '
                         'AND local_name=? AND remote_parent_ref=?',
    ...
    def update_local_state(self, row, info, versioned=True, queue=True):
        ...
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                'UPDATE States SET last_local_updated = ?, local_digest = ?, '
                'local_path = ?, local_parent_path = ?, local_name = ?, '
                'local_state = ?, size = ?, remote_state = ?, '
                'pair_state = ? {version} '
    ...
    def get_valid_duplicate_file(self, digest):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE remote_digest=? '
                         "AND pair_state='synchronized'", (digest,)).fetchone()
    ...
    def get_remote_children(self, ref):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE remote_parent_ref=?',
                         (ref,)).fetchall()

    def get_new_remote_children(self, ref):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute(
            'SELECT * FROM States WHERE remote_parent_ref=? '
            "AND remote_state='created' AND local_state='unknown'",
            (ref,)).fetchall()
    ...
    def get_syncing_count(self, threshold=3):
        count = self.get_count(
            "pair_state!='synchronized' AND pair_state!='conflicted' "
            "AND pair_state!='unsynchronized' AND error_count < {}".format(
                str(threshold)))
    ...
    def get_global_size(self):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT SUM(size) as sum FROM States '
                         "WHERE pair_state='synchronized'").fetchone().sum

    def get_unsynchronizeds(self):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States '
    ...
    def get_errors(self, limit=3):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE error_count > ?',
                         (limit,)).fetchall()

    def get_local_children(self, path):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE local_parent_path=?',
                         (path,)).fetchall()

    def get_states_from_partial_local(self, path):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE local_path LIKE ?',
                         ('{}%'.format(path),)).fetchall()

    def get_first_state_from_partial_remote(self, ref):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE remote_ref LIKE ? '
                         'ORDER BY last_remote_updated ASC LIMIT 1',
                         ('%{}'.format(ref),)).fetchone()
    ...
    def get_state_from_remote_with_path(self, ref, path):
        ...
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE remote_ref=? '
                         'AND remote_parent_path=?', (ref, path)).fetchone()
    ...
    def get_state_from_id(self, row_id, from_write=False):
        ...
        try:
            ...
            state = c.execute('SELECT * FROM States WHERE id=?',
                              (row_id,)).fetchone()
        finally:
            if from_write:
                self._lock.release()
        ...
        with self._lock:
            ...
            if doc_pair.folderish:
                path = '{}/{}'.format(doc_pair.remote_parent_path,
                                      doc_pair.remote_ref)
                query = ('UPDATE States SET remote_parent_path={}/{} || '
                         'substr(remote_parent_path,{}) {}').format(
                ...
                c.execute(query)
            c.execute('UPDATE States SET remote_parent_path=? WHERE id=?',
    ...
    def update_local_parent_path(self, doc_pair, new_name, new_path):
        with self._lock:
            ...
            if doc_pair.folderish:
                ...
                query = (
                    "UPDATE States SET local_parent_path='{0}' || "
                    "substr(local_parent_path, {1}), local_path='{0}' || "
                    'substr(local_path, {1}) {2}'.format(
                        path, count, self._get_recursive_condition(doc_pair)))
                c.execute(query)
            # Dont need to update the path as it is refresh later
            c.execute('UPDATE States SET local_parent_path=? WHERE id=?',
                      (new_path, doc_pair.id))
            if self.auto_commit:
                con.commit()
    ...
    def mark_descendants_remotely_created(self, doc_pair):
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            update = ('UPDATE States SET local_digest=NULL, '
                      'last_local_updated=NULL, local_name=NULL, '
                      "remote_state='created', pair_state='remotely_created'")
    ...
    def get_state_from_local(self, path):
        c = self._get_read_connection(factory=self._state_factory).cursor()
        return c.execute('SELECT * FROM States WHERE local_path=?',
                         (path,)).fetchone()
    ...
    def insert_remote_state(self, info, remote_parent_path, local_path,
        ...
        with self._lock:
            ...
            c.execute('INSERT INTO States (remote_ref, remote_parent_ref, '
                      'remote_parent_path, remote_name, last_remote_updated, '
                      'remote_can_rename, remote_can_delete, '
                      'remote_can_update, remote_can_create_child, '
                      'last_remote_modifier, remote_digest, folderish, '
                      'last_remote_modifier, local_path, local_parent_path, '
                      'remote_state, local_state, pair_state, local_name) '
                      'VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'
                      "'created','unknown',?, ?)",
            ...
            parent = c.execute('SELECT * FROM States WHERE remote_ref=?',
    ...
    def queue_children(self, row):
        with self._lock:
            ...
            children = c.execute(
                'SELECT * FROM States WHERE remote_parent_ref=? '
                'OR local_parent_path=? AND {}'.format(
    ...
    def increase_error(self, row, error, details=None, incr=1):
        ...
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                'UPDATE States SET last_error=?, last_sync_error_date=?, '
                'error_count = error_count + ?, last_error_details=? '
    ...
    def reset_error(self, row, last_error=None):
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            c.execute('UPDATE States SET last_error=?, '
                      'last_error_details=NULL, last_sync_error_date=NULL, '
                      'error_count=0 WHERE id=?', (last_error, row.id))
            if self.auto_commit:
                con.commit()
    ...
    def force_remote(self, row):
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                "UPDATE States SET local_state='synchronized', "
                "remote_state='modified', pair_state='remotely_modified', "
                'last_error=NULL, last_sync_error_date=NULL, error_count = 0'
    ...
    def force_local(self, row):
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                "UPDATE States SET local_state='resolved', "
                "remote_state='unknown', pair_state=?, last_error=NULL, "
                'last_sync_error_date=NULL, error_count = 0'
    ...
    def unsynchronize_state(self, row, last_error=None):
        with self._lock:
            ...
            c = con.cursor()
            c.execute(
                "UPDATE States SET pair_state='unsynchronized', "
                'last_sync_date=?, processor = 0, last_error=?, '
                'error_count=0, last_sync_error_date=NULL WHERE id=?',
    ...
    def synchronize_state(self, row, version=None, dynamic_states=False):
        ...
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            c.execute('UPDATE States SET local_state = ?, remote_state = ?, '
                      'pair_state = ?, local_digest = ?, last_sync_date = ?, '
                      'processor = 0, last_error = NULL, '
                      'last_error_details = NULL, error_count = 0, '
                      'last_sync_error_date = NULL WHERE id = ? '
                      'AND version = ?',
        ...
        if not result and row.folderish:
            with self._lock:
                ...
                c = con.cursor()
                c.execute(
                    'UPDATE States SET local_state = ?, remote_state = ?, '
                    'pair_state = ?, last_sync_date = ?, processor = 0, '
                    'last_error = NULL, error_count = 0, '
                    'last_sync_error_date = NULL WHERE id = ? '
                    'AND local_path = ? AND remote_name = ? '
                    'AND remote_ref = ? AND remote_parent_ref = ?',
                    (row.local_state, row.remote_state, row.pair_state,
                    datetime.utcnow(), row.id, row.local_path,
                    row.remote_name, row.remote_ref, row.remote_parent_ref))
                if self.auto_commit:
                    con.commit()
            result = c.rowcount == 1
    ...
    def update_remote_state(self, row, info, remote_parent_path=None,
        ...
        if versioned:
            version = ', version=version+1'
            log.trace(
                'Increasing version to %d for pair %r', row.version + 1, row)
        with self._lock:
            con = self._get_write_connection()
            c = con.cursor()
            query = ('UPDATE States SET remote_ref=?, remote_parent_ref=?, '
                     'remote_parent_path=?, remote_name=?, '
                     'last_remote_updated=?, remote_can_rename=?,'
            ...
            query += ('local_state=?, remote_state=?, pair_state=? {} '
                      'WHERE id=?'.format(version))
            c.execute(query,
                      (info.uid, info.parent_uid, remote_parent_path,
                       info.name, info.last_modification_time,
                       info.can_rename, info.can_delete, info.can_update,
    ...
    def add_path_to_scan(self, path):
        ...
        try:
            with self._lock:
                ...
                c.execute('DELETE FROM ToRemoteScan WHERE path LIKE ?',
                          (path + '%',))
                # ADD IT
    ...
    def get_batch_sync_ignore():
        return ("AND (pair_state != 'unsynchronized' "
                "AND pair_state != 'conflicted') AND folderish=0 ")

    def get_adjacent_sync_file(self, ref, comp, order, sync_mode=None):
        ...
        c = self._get_read_connection().cursor()
        return c.execute(
            ('SELECT * FROM States WHERE last_sync_date {} ? {}{} '
             'ORDER BY last_sync_date {} LIMIT 1').format(
                comp, mode, self.get_batch_sync_ignore(), order),
            (state.last_sync_date,)).fetchone()

    def get_previous_sync_file(self, ref, sync_mode=None):
        return self.get_adjacent_sync_file(ref, '>', 'ASC', sync_mode)

    def get_next_sync_file(self, ref, sync_mode=None):
        return self.get_adjacent_sync_file(ref, '<', 'DESC', sync_mode)

    def get_adjacent_folder_file(self, ref, comp, order):
        state = self.get_normal_state_from_remote(ref)
        c = self._get_read_connection().cursor()
        return c.execute(
            ('SELECT * FROM States WHERE remote_parent_ref=? '
             'AND remote_name {} ? AND folderish=0 ORDER BY '
             'remote_name {} LIMIT 1').format(comp, order),
            (state.remote_parent_ref, state.remote_name)).fetchone()

    def get_next_folder_file(self, ref):
        return self.get_adjacent_folder_file(ref, '>', 'ASC')

    def get_previous_folder_file(self, ref):
        return self.get_adjacent_folder_file(ref, '<', 'DESC')
